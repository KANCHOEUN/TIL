...
Call-by-reference : 함수의 인자로 주소값을 넘겼기 때문에 reference가 아니라, 주소값을 이용해서 외부에 선언된 메모리 공간에 접근할 수 있는 형태이기 때문에 reference
...
// 외부에 선언된 변수의 메모리 공간에 접근할 수 있으므로 Call-by-reference
SwapByRef(&num1, &num2);
```
...
int *p2 = &num2;
SwapByRef(p1, p2);
// num1과 num2의 관점에서는 주소값을 전달했으므로 Call-by-reference
// ptr1과 ptr2 관점에서는 주소값이 아닌 값을 전달했으므로 Call-by-value
```
...
매개 변수는 함수가 호출될 때 선언이 되는 변수이다.

함수 호출의 과정에서 선언과 동시에 전달되는 대상으로 초기화한다.

즉 매개변수에 선언된 참조자도 선언과 동시에 초기화되므로 참조자의 기준에 위배되지 않는다.



하나의 메모리 공간에 두 개의 이름이 붙었는데 그 두 개의 이름이 각각 존재하는 스택이 다르다.
...
C 에서는 함수의 호출 형태만 봐도 값이 변경 유무를 알 수 있다.

C ++ 에서는 함수의 정의 형태와 함수의 호출 형태를 보아도 값이 전달이 될지 참조자가 전달이 될지 알 수 없어서

함수의 몸체 부분을 확인해 보아야 값의 변경 유무를 알 수 있다.
...
상수를 메모리 공간에 임시적으로 저장하기 때문에 행을 바꿔도 소멸시키지 않는다.
...
num을 전달을 했는데 참조 & 로 받으면 참조 변수로 받으면 값이 복사

받는 사람이 어떻게 받느냐에 따라서 전달되는 형태가 달라진다.
...
함수 호출 할 때 전달하는 대상을 참조로 받느냐 변수로 받느냐에 따라서 형태가 달라지고

반환을 할 때  참조형으로 반환하느냐 기본 자료형으로 반환하느냐

반환돼서 넘어온 값을 변수에 저장을 하느냐 참조자에 저장을 하느냐
